...
class MaintenanceReportModel extends BaseModel {
  ...
  /**
   * Override addData to bypass BaseModel's hardcoded created_by field insertion
   * The maintenance_report table doesn't have created_by column (uses created_at/updated_at instead)
   * Solution: Use direct SQL INSERT instead of BaseModel.addData which tries to always insert created_by
   * @param {Object} data - Data to insert
   * @param {Object} eventContext - Event context (optional)
   * @returns {Object} Result from direct INSERT
   */
  async addData(data, eventContext = null) {
    if (!data || typeof data !== 'object') {
      return super.addData(data, eventContext);
    }

    // Auto-populate location fields based on facility_id if provided
    if (data.facility_id && (!data.location_floor || !data.location_room)) {
      try {
        const facilityQuery = `SELECT floor_level, room_name, location_detail FROM facility_asset WHERE facility_id = $1`;
        const resFacility = await db.executeQuery(facilityQuery, [data.facility_id]);
        if (resFacility && resFacility.length > 0) {
          const fac = resFacility[0];
          data.location_floor = data.location_floor || fac.floor_level;
          data.location_room = data.location_room || fac.room_name;
          if (!data.location_detail && fac.location_detail) {
             data.location_detail = fac.location_detail;
          }
        }
      } catch (err) {
        console.error('[MAINTENANCE-REPORT] Failed to fetch facility info for auto-population', err.message);
      }
    }

    // Remove created_by field if present (BaseModel would try to insert it)
    const sanitizedData = {};
    ...
    console.log('[MAINTENANCE-REPORT] addData: Using direct SQL INSERT (bypassing BaseModel to avoid created_by error)');
    console.log('[MAINTENANCE-REPORT] addData: Data keys:', Object.keys(sanitizedData));

    try {
      // Build direct INSERT query to avoid BaseModel adding created_by
      const columns = Object.keys(sanitizedData);
      const placeholders = columns.map((_, i) => `$${i + 1}`).join(', ');
      const values = columns.map(col => sanitizedData[col]);
      
      const insertQuery = `INSERT INTO ${this.writeSource} (${columns.join(', ')}) VALUES (${placeholders}) RETURNING *`;
      
      console.log('[MAINTENANCE-REPORT] Direct SQL:', insertQuery);
      console.log('[MAINTENANCE-REPORT] Values:', values);
      ...
      console.error('[MAINTENANCE-REPORT] Direct INSERT failed:', error.message);
      throw error;
    }
  }

  /**
   * Override updateData to bypass BaseModel's hardcoded created_by field insertion
   * The maintenance_report table doesn't have created_by column, so direct SQL keeps update flow aligned with schema.
   * @param {Object} data - Data to update
   * @param {Object} eventContext - Event context (optional)
   * @returns {Object} Result from direct UPDATE
   */
  async updateData(data, eventContext = null) {
    if (!data || typeof data !== 'object') {
      return super.updateData(data, eventContext);
    }
    ...
    if (!recordId) {
      throw new Error(`Primary key (${primaryKey}) is required for update`);
    }

    const sanitizedData = {};
    for (const key in data) {
      if (key !== 'created_by' && key !== primaryKey && this.validFields.includes(key)) {
        sanitizedData[key] = data[key];
      }
    }

    console.log('[MAINTENANCE-REPORT] updateData: Using direct SQL UPDATE (bypassing BaseModel to avoid created_by error)');
    console.log('[MAINTENANCE-REPORT] updateData: Data keys:', Object.keys(sanitizedData));

    try {
      const columns = Object.keys(sanitizedData);
      if (columns.length === 0) {
        const existing = await db.executeQuery(
          `SELECT * FROM ${this.writeSource} WHERE ${primaryKey} = $1 LIMIT 1`,
          [recordId]
        );
      ...
      const updateQuery = `UPDATE ${this.writeSource} SET ${setClause} WHERE ${primaryKey} = $${values.length} RETURNING *`;

      console.log('[MAINTENANCE-REPORT] Direct UPDATE SQL:', updateQuery);
      console.log('[MAINTENANCE-REPORT] Values:', values);
      ...
      console.error('[MAINTENANCE-REPORT] Direct UPDATE failed:', error.message);
      throw error;
    }
  }
